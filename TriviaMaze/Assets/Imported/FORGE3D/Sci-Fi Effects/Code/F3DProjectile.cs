...
public class F3DProjectile : MonoBehaviour
{
	...
	{
		Transform root = GameTools.GetRootTransform (this.transform);
		f3dxC = root.GetComponentInChildren<F3DFXController>();
		//print (f3dxC);
	}
            ...
            if (!isFXSpawned)
            {
                ...
                switch (fxType)
                {
                    case F3DFXType.Vulcan:
						f3dxC.VulcanImpact(hitPoint.point + hitPoint.normal * 0.2f);
                        ApplyForce(2.5f);
                        break;

                    case F3DFXType.SoloGun:
						f3dxC.SoloGunImpact(hitPoint.point + hitPoint.normal * 0.2f);
                        ApplyForce(25f);
                        break;

                    case F3DFXType.Seeker:
						f3dxC.SeekerImpact(hitPoint.point + hitPoint.normal * 1f);
                        ApplyForce(30f);
                        break;

                    case F3DFXType.PlasmaGun:
						f3dxC.PlasmaGunImpact(hitPoint.point + hitPoint.normal * 0.2f);
                        ApplyForce(25f);
                        break;

                    case F3DFXType.LaserImpulse:
						f3dxC.LaserImpulseImpact(hitPoint.point + hitPoint.normal * 0.2f);
                        ApplyForce(25f);
                        break;
